...
class ValidateFullBurntAreasProcess(QgsProcessingAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, model_feedback):
        ...
        startDate = (
            self.parameterAsDateTime(parameters, "StartDate", context)
            .toPyDateTime()
            .strftime("%Y-%m-%d")
        )
        endDate = (
            self.parameterAsDateTime(parameters, "EndDate", context)
            .toPyDateTime()
            .strftime("%Y-%m-%d")
        )
        ...
        try:
            ...
            results = processing.run(
                "BurntAreas:FullBurntAreasProcess",
                results,
                context=context,
                feedback=feedback,
        ...
        except FsidServiceError as serviceError:
            serviceError.processingFeedbackError(feedback)
            results["FsidServiceError"] = serviceError
