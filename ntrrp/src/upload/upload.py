        ...
        try:
            with open(file_path, 'rb') as file:
                ...
                chunk_count = math.ceil(
                    float(file_size) / self.max_byte_length)
                ...
                while True:
                    ...
                    try:
                        ...
                        print('Error while sending chunk to server. Retrying in {} seconds'.format(
                            retry_timeout))
    ...
    try:
        with ZipFile(name, 'r') as f:
            ...
            if ftest is None:
                ...
                print('The file \'%s\' in the archive \'%s\' is corrupted' %
                      (ftest, name))
                return False
...
def main(argv):
    ...
    mainGroup.add_argument('-u', '--url', dest='url', metavar='UPLOAD URL',
                           default='https://test.firenorth.org.au', help='Upload server url')
    mainGroup.add_argument('-f', '--file', dest='file', metavar='ZIP ARCHIVE',
                           default=None, help='The zip file to be uploaded')
    mainGroup.add_argument('-cs', '--chunksize', dest='chunksize', metavar='CHUNK SIZE',
                           type=int, default=1024, help='The max chunk size during upload')

    mainGroup.add_argument('-v', '--verbose', default=False,
                           action='store_true', help='verbose, debug mode')

    mainGroup.add_argument('--version', dest='version',
                           help='Displays script version/revision', default=False, action='store_true')
