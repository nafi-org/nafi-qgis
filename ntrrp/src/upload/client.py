...
# Replace all 'print' statements from Patrice's script with qgsDebug output instead
    ...
    def upload_file(self, file_path, debug=False):
        ...
        try:
            with open(file_path, 'rb') as file:
                ...
                qgsDebug(f"Total chunk count: {chunk_count}")
                retry_timeout = 1
                sent_chunk_count = 0
                ...
                while True:
                    ...
                    headers['Content-Range'] = "bytes={}-{}/{}".format(start, end-1, file_size)
                    ...
                    try:
                        response = requests.put(upload_endpoint, headers=headers, data=data)
                        qgsDebug(response.text)
                        if response.ok:
                            if debug:
                                contentRangeHeader = headers["Content-Range"]
                                qgsDebug(f"[Content-Range] => {contentRangeHeader}")
                                qgsDebug(f"{sent_chunk_count + 1} chunks sent to server for a total of {end / 1024} kbytes so far....")
                            sent_chunk_count += 1
                        else:
                            qgsDebug(f"Error: {response.status_code}")

                    except requests.exceptions.RequestException:
                        qgsDebug(f"Error while sending chunk to server. Retrying in {retry_timeout} seconds")
    ...
    try:
        with ZipFile(name, 'r') as f:
            ...
            if ftest is None:
                ...
                qgsDebug(f"The file \'{ftest}\' in the archive \'{name}\' is corrupted")
                return False
...
#     mainGroup.add_argument('-cs', '--chunksize', dest='chunksize', metavar='CHUNK SIZE', type=int, default=1024, help='The max chunk size during upload')

#     mainGroup.add_argument('-v', '--verbose', default=False, action='store_true', help='verbose, debug mode')
...
#             raise InitializationException('You need to specify a valid upload ur:l. Found \'%s\'' % args.url)
...
#     qgsDebug('Uploading archive \'%s\' to \'%s\'' % (archive, url))
